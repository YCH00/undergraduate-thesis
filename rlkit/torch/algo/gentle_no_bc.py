...
class GENTLE(OfflineMetaRLAlgorithm):
    ...
    def sample_sac(self, indices):
        ...
        # return [meta_batch * batch_size, dim]
    ...
    def sample_context(self, indices, b_size=None):
        ...
        # return [meta_batch, embedding_batch_size, context_dim]
    ...
    def _take_step(self, indices, context):
        ...
        perm = torch.randperm(len(z), device=ptu.device)
        z_perm = z[perm]  # (z, z_perm)即为随机配对的(z_i, z_j)
        alpha = torch.rand(len(z), 1, device=ptu.device)
        # print('new_actions: ', new_actions.shape)
        # print('z: ', type(z), z.shape)
        # print(z)
        # print('z_perm: ', type(z_perm), z_perm.shape)
        ...
        z_t_expanded = z_t.view(num_tasks, batch_size, -1)  # z_t_expanded: [num_tasks, batch_size, z_dim]
        # print('z_t: ', z_t.shape)
        # print('z_t_expand: ', z_t_expanded.shape)
        # exit(0)
        
        action_tilde = self.agent.get_policy_action(
            obs = obs,
            given_z = z_t)

        # print('action_tilde: ', action_tilde.shape)
        # exit(0)
        action_tilde_expanded = action_tilde.view(num_tasks, batch_size, -1)
        
        # YinCH_todo: 这里 FOCAL_Decoder 似乎并没有forward函数  -- done
        with torch.no_grad():
            next_obs_tilde, reward_tilde = self.context_decoder(
                obs,
                action_tilde_expanded,
                z_t_expanded
            )
        # print('next_obs_tilde: ', next_obs_tilde.shape) 
        # print('reward_tilde: ', reward_tilde.shape)
        # exit(0)
        ...
        #     task_indices=indices
        # )
        z_hat = self.agent.get_z(virtual_context)  # [num_tasks*batch_size, dim]
        z_dim = z_hat.size(-1)
        # YinCH_todo: z_hat上在每一个任务上取均值
        z_hat_task = z_hat.view(num_tasks, batch_size, -1)
        z_task_mean = z_hat_task.mean(dim=1)
        z_task_mean_expand = z_task_mean.unsqueeze(1).expand(-1, batch_size, -1)
        ...
        # # print('tmp: ', tmp.shape)
        # # exit(0)
        # z_task = z.view(num_tasks, batch_size, -1).mean(dim=1)   # [T, D]
        # z_task = z_task.unsqueeze(1).repeat(1, c_b, 1)
        # print('z_task: ', z_task.shape)

        # r_next_s = context[...,obs_dim+action_dim:]
        # pred_next_s, pred_r = self.context_decoder(context[...,:obs_dim], context[...,obs_dim:obs_dim+action_dim], z_task)
        # pred_r_next_s = torch.cat([pred_next_s, pred_r], dim=-1)
        # # print('r_next_s: ', r_next_s.shape)
        # # print('pred_next_s: ', pred_next_s.shape)
        ...
        # print(context_loss.requires_grad)
        # exit(0)
        ...
        if self.eval_statistics is None:
            ...
            self.eval_statistics['Consistency Loss'] = np.mean(ptu.get_numpy(
                consistency_loss
            ))
